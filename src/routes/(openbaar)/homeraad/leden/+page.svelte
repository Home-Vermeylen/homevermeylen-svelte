<script lang="ts">
	...
	console.log(data.academiejaar_query)
</script>

<div class="flex flex-col items-center">
	<div class="flex flex-col items-center text-center p-10">
		<h1 class="py-5 text-3xl font-bold italic tracking-wider">
			Academiejaar 
		</h1>
	...
	<div class="grid gap-x-10 gap-y-10 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
		{#each data.praesidium_leden as praesidiumlid (praesidiumlid.id)}
			<Praesidiumlid
				lid={praesidiumlid}
				isHuidigPraesidium={data.academiejaar_query == null}
			/>
		{/each}
	</div>
</div>
