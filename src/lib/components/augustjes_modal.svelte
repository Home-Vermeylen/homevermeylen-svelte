<script lang="ts">
	...
	$effect(() => {
		if ($page.state.augustje) {
			$formData.naam = $page.state.augustje.naam ?? '';
			$formData.id = $page.state.augustje.id;
		}
	});
</script>
...
>
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title>
				{#if $page.state.augustje != null}
					<strong>{$page.state.augustje?.naam}</strong> bewerken
				{:else}
					Nieuw augustje
				{/if}
			</Dialog.Title>
